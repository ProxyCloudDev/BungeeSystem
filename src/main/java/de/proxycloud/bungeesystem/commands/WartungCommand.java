...
public class WartungCommand extends Command
{
    ...
    public void execute(CommandSender commandSender, String[] strings)
    {
        if(commandSender instanceof ProxiedPlayer)
        {
            ...
            if(strings.length != 1)
            {
                player.sendMessage(new TextComponent(BungeeSystem.getInstance().getPrefix() + "§7Benutze§8: §8/§awartung §8<§aan§8/§aaus§8>"));
                return;
            }
        ...
        else
        {
            if(strings.length != 1)
            {
                commandSender.sendMessage(new TextComponent(BungeeSystem.getInstance().getPrefix() + "§7Benutze§8: §8/§awartung §8<§aan§8/§aaus§8>"));
                return;
            }
            if(strings[0].equalsIgnoreCase("an"))
            {
